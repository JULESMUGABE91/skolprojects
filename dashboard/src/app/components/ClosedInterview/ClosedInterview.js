...
class ClosedInterview extends React.Component {
  state = {
    data: [],
    isLoading: true,
    user: {},
  ...
  getData(isLoading) {
    ...
    let url = ENDPOINT + "/answer/closedInterviews";
    ...
    axios(options)
      .then((res) => {
        ...
        saveDownload({
          ...
          user: "ClosedInterview-",
        });

        toastMessage("success", "Download completed");
      })
      .catch((error) => {
        console.log(error);
        this.setState({
...
export default connect(mapPropsToState)(ClosedInterview);
